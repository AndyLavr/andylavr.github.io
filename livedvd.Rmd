---
title: "Dragon Live DVD/USB stick Ubuntu/Kubuntu/Xubuntu/Lubuntu (Cosmic/Disco)"
permalink: livedvd/
---
[![standard-readme compliant](https://img.shields.io/badge/readme%20style-standard-brightgreen.svg)](https://github.com/RichardLitt/standard-readme)
[![Donate with Bitcoin](https://en.cryptobadges.io/badge/micro/1KvWrbLhuzk8DSb2Yq2948bMj3uQvVTQCW)](https://en.cryptobadges.io/donate/1KvWrbLhuzk8DSb2Yq2948bMj3uQvVTQCW)
[![Donate with Ethereum](https://en.cryptobadges.io/badge/micro/0xE42f2d4D4aF888Ac784ED26a68E828475A4712Cb)](https://en.cryptobadges.io/donate/0xE42f2d4D4aF888Ac784ED26a68E828475A4712Cb)

---

```
WARNING!

 You can only use kernel packages that are in PPA if you have a Dragon Team license key!
Without registering with the Dragon Team you need to do the build kernel packages yourself.
If you do not register at Dragon Team, the kernel installed from the PPA will work for only 15 minutes ...
```

 - For registration in team [please read information](https://dragon-kernel.pro/licensing).


**[Burning Iso Howto](https://dragon-kernel.pro/burn)**


** Info for boot UEFI32 (example for Acer Aspire SW5-012)

```
 1. Go to BIOS settings with F2
 2. Add the bootia32.efi file to the trusted UEFI files by selecting it and naming the option to whatever you want.
 3. Enable F12 boot menu.
 4. Exit saving changes.
 5. Tap F12 until you get boot menu.
 6. You will see your boot option there.

 - The bootia32.efi file and grub-efi-ia32 package is already in the image.
```
---


**Lubuntu Dragon Live DVD for Atom CPU x86_64 - INTEL Bay[Cherry]Trail**

 - Warning! It works only with an activated Dragon Team members license!

 - Kernel version [4.20.7-dragon Release](https://dragon-kernel.pro), software for Live DVD upgraded to **20190208**


[![Download Atom Live DVD image](https://img.shields.io/badge/Download-LiveUSB-green.svg)](https://drive.google.com/open?id=1W76vzxZrhUD2QQXcC6bXsBti65ta25VP)


**Lubuntu Dragon Live DVD for GENERIC CPU x86_64**

 - Warning! It works only with an activated Dragon Team members license!

 - Kernel version [4.20.7-dragon Release](https://dragon-kernel.pro), software for Live DVD upgraded to **20190208**


[![Download Generic Live DVD image](https://img.shields.io/badge/Download-LiveUSB-green.svg)](https://drive.google.com/open?id=1AX0Hn2LHvTpmo5zypl1TAJRWEWWRc_Do)


***


## Donate

[![Donate with Bitcoin](https://en.cryptobadges.io/badge/big/1KvWrbLhuzk8DSb2Yq2948bMj3uQvVTQCW)](https://en.cryptobadges.io/donate/1KvWrbLhuzk8DSb2Yq2948bMj3uQvVTQCW)
[![Donate with Ethereum](https://en.cryptobadges.io/badge/big/0xE42f2d4D4aF888Ac784ED26a68E828475A4712Cb)](https://en.cryptobadges.io/donate/0xE42f2d4D4aF888Ac784ED26a68E828475A4712Cb)

***

