---
title: "Signing a custom kernel for Secure Boot"
permalink: sign/
---
...
please inform yourself on how to install it correctly, so you do not mess up your bootloader.
 ...
 Since the most recent GRUB2 update (2.02) in Ubuntu, GRUB2 does not load unsigned
...
7. Sign your installed kernel (it should be at /boot/vmlinuz-[KERNEL-VERSION]-dragon-generic):

```
$ sudo sbsign --key MOK.priv --cert MOK.pem /boot/vmlinuz-[KERNEL-VERSION]-dragon-generic --output /boot/vmlinuz-[KERNEL-VERSION]-dragon-generic.signed
```

8. Copy the initram of the unsigned kernel, so we also have an initram for the signed one.

```
$ sudo cp /boot/initrd.img-[KERNEL-VERSION]-dragon-generic{,.signed}
```
...
```
$ sudo mv /boot/vmlinuz-[KERNEL-VERSION]-dragon-generic{.signed,}
